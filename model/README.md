...
![cinética del sensor](../figures/fig1_sensor.png)
...
*El barrido completo está en `model/figures/04_toxin_sweep.png`, ahora Extended Data ED1.*
...
![kill switch](../figures/fig2_killswitch.png)
...
![sensibilidad](../figures/fig3_sensitivity.png)
...
Las seis se generan con el mismo contrato, pensado para columna de manuscrito:

- ancho fijo de **89 mm** (columna simple) o **183 mm** (doble)
- **PDF vectorial** con texto editable, más PNG a 600 dpi de vista previa
- piso de **5 pt** para todo glifo, incluidos sub y superíndices de mathtext
- paneles rotulados `a`, `b`, `c`… con desplazamiento en puntos, no en
  fracción de eje, para que no se muevan entre paneles de distinta altura

Cada export corre tres auditorías y **ninguna es opcional**: alineación de
paneles (bordes y canales dentro de 1.5 pt), tamaño de glifo y colisiones de
texto sobre el PDF final. Un fallo aborta el export en vez de escribir una
figura defectuosa. Los `*.alignment.json` y `*.collision-audit.json` que quedan
junto a cada figura son el registro de esas auditorías.

Las auditorías viven en la skill `nature-figure`. Si no está instalada, el
export sigue adelante pero avisa por stderr que la figura **no** quedó
auditada. Para apuntar a otra ruta: `NATURE_FIGURE_SCRIPTS=/ruta/a/scripts`.
...
.venv/bin/python model/sensitivity.py --fast
```
El barrido de Sobol cuesta minutos; las figuras se reajustan en segundos. Por
eso las muestras se cachean en `.cache/`, indexadas por los rangos de
`UNCERTAINTY` y el tamaño de muestra: cambiar un rango invalida la caché,
cambiar el estilo de una figura no. `--refresh` la ignora y recorre el modelo.
